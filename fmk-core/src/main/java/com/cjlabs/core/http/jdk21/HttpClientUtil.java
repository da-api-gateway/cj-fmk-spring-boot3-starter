...
public class HttpClientUtil {
    ...
    private static volatile HttpConfig config;
     ...
     * 修改配置并重建 HttpClient
     */
    public static synchronized void updateConfig(HttpConfig newConfig) {
        config = newConfig;

        // 如果配置了代理，则设置代理
        if (newConfig.isUserProxy()) {
    ...
    private static void rebuildClient() {
        client = HttpClient.newBuilder()
                ...
                .version(config.isUseHttp2() ? HttpClient.Version.HTTP_2 : HttpClient.Version.HTTP_1_1)
                .build();
    }
        ...
        if (config.isPrintJson()) {
            ...
            if (MapUtils.isNotEmpty(headers)) {
                ...
                headers.forEach((name, values) -> {
                    log.info("HttpClientUtil|logResponseHeaders|{} = {}", name, String.join(", ", values));
                });
            }
        }
    }
